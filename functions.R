...
plot_volcano <- function(de_table, cell_type, gene_name) {
  ...
  p +
    ...
    geom_text_repel(data=de.dt[de.dt$colour == "selected", ], aes(label = Gene), colour="red", size=5)
}
...
dotPlot <- function (gene_name = "Scgb1a1"){
    ...
    expression <- h5read("data/AgingData.h5", name = as.character(gene_name))
    ...
    theme(axis.text.y = element_text(size=9), plot.margin=unit(c(1,1,1,1), "cm"), legend.text=element_text(size=8), legend.title=element_text(size=8), legend.position = c(0.75, 0.5), axis.title.x = element_blank(), axis.title.y = element_blank())
    p
  }
